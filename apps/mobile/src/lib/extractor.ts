// Task extraction pipeline (mobile).
//
// Mirrors apps/web/src/lib/extractor.ts. Same layers, same import of
// matchEmail + SENDER_CATALOG from @inbox/shared. Only difference: we
// don't rely on crypto.randomUUID() (not available in React Native
// without a polyfill); IDs are minted with timestamp + random suffix.
//
...
// ---------------------------------------------------------------------------
// ID minting — RN-safe, no crypto.randomUUID dependency
// ---------------------------------------------------------------------------
 ...
 * more tasks. Carries the `createdAt`-equivalent via `id` (fresh
 * timestamp+random) and `source_email_id` (gmailId), as the orchestrator
 * caller expects.
 *
 * Layers are tried in order; the first that matches wins.
 */

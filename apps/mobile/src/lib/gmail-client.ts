// Gmail REST API client (mobile).
//
// Mirrors apps/web/src/lib/gmail-client.ts. React Native ships a global
// fetch that speaks the same surface as the browser's, so the wire calls
// are unchanged. We avoid `atob` (not always present in RN) by decoding
// base64url via Buffer / Uint8Array.
//
// Public surface:
//   - listRecentMessages(accessToken, opts?) -> GmailMessage[]
//   - getMessage(accessToken, messageId)     -> ParsedEmail
//   - TokenExpired (Error subclass)
//
...
// React-Native-safe base64url decoder. Avoids `atob` (missing in some RN
// builds) by mapping each char through the base64 alphabet manually, then
// decoding the resulting byte stream as UTF-8.
const B64_ALPHABET =
...
function decodeBase64Url(data: string): string {
  ...
  for (let i = 0; i < padded.length; i += 4) {
    const c1 = B64_ALPHABET.indexOf(padded[i]);
    const c2 = B64_ALPHABET.indexOf(padded[i + 1]);
    const c3 = padded[i + 2] === "=" ? -1 : B64_ALPHABET.indexOf(padded[i + 2]);
    const c4 = padded[i + 3] === "=" ? -1 : B64_ALPHABET.indexOf(padded[i + 3]);
  ...
  // Fallback: percent-decode via encodeURIComponent trick.
  let s = "";

// Demo email fixture + onboarding helpers.
//
// Drives "Demo Mode" on the dashboard: a hand-curated set of 10 emails that
// exercises every catalog category and the filter fallback. Used by the
// onboarding flow to show what the agent does before the user connects
// Gmail, and by tests to assert end-to-end extraction behaviour.
//
 ...
 * produces to IndexedDB. Marks each demo email as seen, even when the
 * extractor returns no tasks (the 2FA + newsletter cases) so re-running the
...
export async function clearDemoTasks(): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    for (const id of demoIds) store.delete(id);
